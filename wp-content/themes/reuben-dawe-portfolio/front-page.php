<?php get_header(); ?>

<!--==================== MAIN ====================-->

<main class="main">

         <!--==================== HOME ====================-->

         <section class="home section" id="home">
            <div class="home__container container grid">
               <h1 class="home__name">
                  Reuben G. Dawe
               </h1>

               <div class="home__perfil">
                  <div class="home__image">
                     <img src="<?php echo get_theme_file_uri('/images/reuben-george-dawe-portfolio-intro-image.png') ?>" alt="image" class="home__img">
                     <div class="home__shadow"></div> 
                  </div>
               </div>

               <div class="home__info">
                  <p class="home__description">
                     <b>Graphic Designer &amp; Front End Developer:</b> with 6+ years experience in print 
                     and digital design, I offer a dynamic range of services that will enhance any business.
                  </p>
                  
                  <a href="#about" class="home__scroll">
                     <div class="home__scroll-box">
                        <i class="ri-arrow-down-circle-fill"></i>
                     </div>
                     
                     <span class="home__scroll-text">Take a look</span>
                  </a>
               </div>
            </div>
         </section>

         <!--==================== ABOUT ====================-->

         <section class="about section" id="about">
            <div class="about__container container grid">
               <h2 class="section__title-1">
                  <span>Bio.</span>
               </h2>

               <div class="about__perfil">
                  <div class="about__image">
                     <img src="<?php echo get_theme_file_uri('/images/reuben-george-dawe-bio-image.png'); ?>" alt="image" class="about__img">

                     <div class="about__shadow"></div>

                  </div>
               </div>

               <div class="about__info">
                  <p class="about__description">
                     I'm ambitious and enthusiastic about upgrading your Business's <b>Website, Applications and Graphics</b>. 

                  <ul class="about__list">
                     <li class="about__item">
                        <b>My Front End Skills Are:</b> PHP, HTML, CSS, JavaScript, 
                        Git, Bootstrap &amp; GSAP - Indepth experience using VScode, PHPStorm &amp; Wordpress.
                     </li>
                     <br />
                     <li>
                        <b>My Design Skills Are:</b> Publication/Print, Brand Identity, 3D, Video, SM Marketing &amp; Merchandise - 
                        Indepth experience using Adobe CC (Photoshop, Illustrator, InDesign, Premiere Pro, After Effects), Canva, Blender &amp; Google SketchUp.
                     </li>
                  </ul>

                  <div class="about__buttons">
                     <a href="#contact" class="button">
                        <i class="ri-mail-send-fill"></i> Reach Out
                     </a>

                     <a href="https://uk.linkedin.com/in/reuben-dawe-221b3a201" target="_blank" class="button__ghost">
                        <i class="ri-linkedin-box-fill"></i>
                     </a>
                  </div>
               </div>
            </div>
         </section>

         <!--==================== SERVICES ====================-->

         <section class="services section">
            <h2 class="section__title-2">
               <span>Services.</span>
            </h2>

            <div class="services__container container grid">
               <article class="services__card">
                  <div class="services__border"></div>

                  <div class="services__content">
                     <div class="services__icon">
                        <div class="services__box"></div>
                        <i class="ri-book-read-fill"></i>
                     </div>
   
                     <h2 class="services__title">Publication Design</h2>
   
                     <p class="services__description">
                        Modern &amp; eye-catching Magazine, Book Cover, Leaflet,
                        Newsletter &amp; Catalogue designs.
                     </p>
                  </div>
               </article>

               <article class="services__card">
                  <div class="services__border"></div>

                  <div class="services__content">
                     <div class="services__icon">
                        <div class="services__box"></div>
                        <i class="ri-artboard-fill"></i>
                     </div>
   
                     <h2 class="services__title">Brand Identity</h2>
   
                     <p class="services__description">
                        Professional Brand Identity design including Brand Guidelines, Logos, Stationery,
                        Packaging &amp; Market Collateral.
                     </p>
                  </div>
               </article>

               <article class="services__card">
                  <div class="services__border"></div>

                  <div class="services__content">
                     <div class="services__icon">
                        <div class="services__box"></div>
                        <i class="ri-blender-fill"></i>
                     </div>
   
                     <h2 class="services__title">3D &amp; Animation</h2>
   
                     <p class="services__description">
                        Practical 3D projects including Animation, Product Mockups &amp; Exhibition Design.
                     </p>
                  </div>
               </article>

               <article class="services__card">
                  <div class="services__border"></div>

                  <div class="services__content">
                     <div class="services__icon">
                        <div class="services__box"></div>
                        <i class="ri-code-box-fill"></i>
                     </div>
   
                     <h2 class="services__title">Web Development</h2>
   
                     <p class="services__description">
                        Contemporary Website &amp; Application Development - Landing Pages, E-stores, Portfolios,
                        Booking Forms, Product Promotion Pages &amp; more.
                     </p>
                  </div>
               </article>

               <article class="services__card">
                  <div class="services__border"></div>

                  <div class="services__content">
                     <div class="services__icon">
                        <div class="services__box"></div>
                        <i class="ri-advertisement-fill"></i>
                     </div>
   
                     <h2 class="services__title">Social Media Marketing</h2>
   
                     <p class="services__description">
                        Attention-grabbing &amp; high-conversion Social Media Advertisements including custom Graphics &amp; Copywriting.
                     </p>
                  </div>
               </article>

               <article class="services__card">
                  <div class="services__border"></div>

                  <div class="services__content">
                     <div class="services__icon">
                        <div class="services__box"></div>
                        <i class="ri-shirt-fill"></i>
                     </div>
   
                     <h2 class="services__title">Merchandise</h2>
   
                     <p class="services__description">
                        Formal merchandise design including T-shirts, Mugs, Tote Bags, Stickers &amp; Hats.
                     </p>
                  </div>
               </article>
            </div>
         </section>

         <!--==================== PROJECTS ====================--> 

            <section class="projects section" id="projects">
               <h2 class="section__title-1">
                  <span>Portfolio.</span>
               </h2>
               
               <div class="projects__container container grid">

                <?php 
                $homepageEvents = new WP_Query(array(
                    'posts_per_page' => -1,
                    'post_type' => 'event'
                ));

                while($homepageEvents->have_posts()) {
                    $homepageEvents->the_post(); ?>
   
                        <article class="projects__card">

                           <div class="projects__image">
                              <img><?php the_post_thumbnail(); ?></img>

                              <a href="https://github.com/pinkdawe/AJ-Holmes-Website" class="projects__button button">
                                 <i class="ri-github-fill"></i>
                              </a>
                           </div>

                           <div class="projects__content">
                              <h3 class="projects__subtitle">Merchandise</h3>
                              <h2 class="projects__title"><a href="<?php the_permalink(); ?>"><?php the_title() ;?></a></h2>

                              <p class="projects__description">
                                 <?php echo get_the_excerpt(); ?>                                    
                              </p>
                           </div>

                           <div class="projects__buttons">
                              <a href="#" target="_blank" class="projects__link">
                                 <i class="ri-instagram-fill"></i> View
                              </a>

                              <!-- <a href="#" target="_blank" class="projects__link">
                                 <i class="ri-computer-fill"></i> View
                              </a> -->
                           </div>
                        </article> 
                <?php } 
                ?>

                </div>
                     </section>

         <!--==================== CONTACT ====================-->

         <section class="contact section" id="contact">
            <div class="contact__container grid">
               <div class="contact__data">
                  <h2 class="section__title-2">
                     <span>Reach Out.</span>
                  </h2>

                  <p class="contact__description-1">
                     For any inquiries, feel free to drop me a message. I will reply within 24 hours.
                  </p>

                  <p class="contact__description-2">
                     I'll need your <b>First Name</b> and <b>Email Address</b>, but 
                     don't worry - you won't receive anything apart from my reply!
                  </p>

                 
               </div>

               <div class="contact__mail">
                  <h2 class="contact__title">
                     Interested? Send me a message
                  </h2>

                  <form action="" class="contact__form" id="contact-form">
                     <div class="contact__group">
                        <div class="contact__box">
                           <input type="text" name="user_name" class="contact__input" id="name" required placeholder="First Name">
                           <label for="name" class="contact__label">First Name</label>
                        </div>
   
                        <div class="contact__box">
                           <input type="email" name="user_email" class="contact__input" id="email" required placeholder="Email Address">
                           <label for="email" class="contact__label">Email Address</label>
                        </div>
                     </div>
                     
                     <div class="contact__box">
                        <input type="text" name="user_subject" class="contact__input" id="subject" required placeholder="Subject">
                        <label for="subject" class="contact__label">Subject</label>
                     </div>

                     <div class="contact__box contact__area">
                        <textarea name="user_message" class="contact__input" id="message" required placeholder="Message"></textarea>
                        <label for="message" class="contact__label">Message</label>
                     </div>

                     <p class="contact__message" id="contact-message"></p>

                     <button type="submit" class="contact__button button">
                        <i class="ri-mail-send-fill"></i> Reach Out
                     </button>
                  </form>
               </div>

               <div class="contact__social">
                  <!-- <img src="assets/img/curved-arrow.svg" alt="" class="contact__social-arrow"> -->

                  <div class="contact__social-data">
                     <div>
                        <p class="contact__social-description-1">
                           Does not send emails
                        </p>
   
                        <p class="contact__social-description-2">
                           Write me on my social networks
                        </p>
                     </div>

                     <div class="contact__social-links">
                        <a href="https://www.instagram.com/dawe.js/" class="contact__social-link" target="_blank">
                           <i class="ri-instagram-fill"></i>
                        </a>
   
                        <a href="https://uk.linkedin.com/in/reuben-dawe-221b3a201" class="contact__social-link" target="_blank">
                           <i class="ri-linkedin-fill"></i>
                        </a>
   
                        <a href="https://github.com/pinkdawe" class="contact__social-link" target="_blank">
                           <i class="ri-github-fill"></i>
                        </a>
                     </div>
                  </div>
               </div>
            </div>
         </section>
      </main>

<?php get_footer(); ?>